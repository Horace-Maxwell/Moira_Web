...
GitHub Actions runs the same production gate, browser layout check, and one-command Docker Compose deploy path on `main`, pull requests, and tagged web releases. It also builds a `linux/amd64` Docker image so release regressions are caught before someone deploys from another server.
