...
grep -q '选择星盘(&M)...' "$HTML_FILE"
grep -q '显示流年(&N)' "$HTML_FILE"
grep -q '显示三垣列宿' "$HTML_FILE"
grep -q '显示单圈(&N)' "$HTML_FILE"
grep -q '显示地平方位' "$HTML_FILE"
grep -q '项目显示' "$HTML_FILE"
grep -q '工具列显示' "$HTML_FILE"
grep -q '选择星曜(&I)...' "$HTML_FILE"
grep -q '选择角距显示(&O)...' "$HTML_FILE"
grep -q '选择相位(&K)...' "$HTML_FILE"
grep -q '选择星曜及强势角距(&J)...' "$HTML_FILE"
grep -q '选择分宫制(&H)...' "$HTML_FILE"
grep -q '选择回归恒星制(&S)...' "$HTML_FILE"
grep -q '选择合盘计算(&U)...' "$HTML_FILE"
grep -q '选择神煞(&S)...' "$HTML_FILE"
grep -q '选择政余格局(&Z)...' "$HTML_FILE"
grep -q 'class="menu-cascade"' "$HTML_FILE"
grep -q '流年星法(&T)...' "$HTML_FILE"
grep -q '现在时间(&N)' "$HTML_FILE"
grep -q '操作说明(&O)' "$HTML_FILE"
